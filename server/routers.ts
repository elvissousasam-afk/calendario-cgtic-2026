...
import { notifyUpcomingEvents, notifyBirthdays, sendDailySummary } from "./notificationService";
...
export const appRouter = router({
  ...
  // ==================== NOTIFICAÇÕES ====================
  notificacoes: router({
    sendUpcoming: publicProcedure
      .input(z.object({ dias: z.number().min(1).max(7).optional() }).optional())
      .mutation(async ({ input }) => {
        const success = await notifyUpcomingEvents(input?.dias ?? 1);
        return { success };
      }),

    sendBirthdays: publicProcedure
      .mutation(async () => {
        const success = await notifyBirthdays();
        return { success };
      }),
  ...
  configuracoes: router({
    ...
    setMultiple: publicProcedure
      .input(z.object({
        ...
        anoInicial: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        const configs: Record<string, string> = {};
        if (input.titulo !== undefined) configs.titulo = input.titulo;
        if (input.subtitulo !== undefined) configs.subtitulo = input.subtitulo;
        if (input.rodape !== undefined) configs.rodape = input.rodape;
        if (input.anoInicial !== undefined) configs.anoInicial = input.anoInicial;
        return await db.setMultipleConfiguracoes(configs);
      }),
  }),
});
